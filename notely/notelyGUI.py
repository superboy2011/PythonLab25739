...
import PyQt5.QtGui
from PyQt5.QtWidgets import QMessageBox, QPushButton, QSizePolicy, QCheckBox, QVBoxLayout
...
class StartWindow(QtWidgets.QMainWindow, Ui_Logged_in_window):
    def __init__(self):
        ...
        self.go_back_btn_3.clicked.connect(partial(self.stackedWidget.setCurrentIndex, 6))
        ...
        self.no_commas = PyQt5.QtGui.QRegExpValidator(QRegExp("[a-zA-z0-9]+"))
    ...
    def add_note_page_render(self, category, prev_page_index):
        ...
        self.discard_note_btn.clicked.connect(partial(self.stackedWidget.setCurrentIndex, prev_page_index))
    ...
    def add_note_gui(self, category, prev_page_index):
        retry = False
        new_note = None
        if self.line_note_name.text() == '':
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("Note name can not be empty")
            msg.setWindowTitle("Wrong Input")
            msg.setStandardButtons(QMessageBox.Ok)
            msg.exec()
            retry = True
        if self.reminder_chkbx.isChecked() and (self.reminder_dt.dateTime().toPyDateTime()-datetime.now()).total_seconds() < 15:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            ...
            msg.exec()
            retry = True
        elif self.reminder_chkbx.isChecked():
    ...
    def add_folder_gui(self):
        if self.new_cat_txt.text() == '':
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("Folder can not be empty")
            msg.setWindowTitle("Wrong Input")
            msg.setStandardButtons(QMessageBox.Ok)
            msg.exec()
        else:
    ...
    def show_all_folders_gui(self):
        ...
        if not success:
            ...
            for idx, category in enumerate(result):
                btn = QPushButton(self.scrollAreaWidgetContents_2)
            ...
            self.scrollAreaWidgetContents_2.setMinimumHeight(20 + 55 * len(result))
    ...
    def show_folder_contents_gui(self, category, prev_page_index):
        ...
        if not success:
            ...
            for idx, note_name in enumerate(result):
                ...
                btn.setGeometry(QtCore.QRect(40, 20 + 55 * idx, 200, 35))
                btn.setSizePolicy(QSizePolicy.Minimum, QSizePolicy.Fixed)
                btn.setObjectName(("btn_cat" + str(idx)))
                btn.show()
            ...
            self.scrollAreaWidgetContents.setMinimumHeight(20 + 55 * len(result))
        ...
        if not success:
            ...
            if result.reminder.year != 1:
                ...
                self.reminder_dt_2.setDateTime(QDateTime.fromString(result.reminder.strftime("%Y %m %d %H %M %S"),
                                                                    'yyyy MM dd hh mm ss'))
                self.reminder_chkbx_2.setDisabled(True)
            else:
    ...
    def update_note_gui(self, old_note, btn, prev_page_index):
        ...
        if self.line_note_name_2.text() == '':
            ...
            msg.exec()
            retry = True
        if self.reminder_chkbx_2.isChecked() and self.reminder_chkbx_2.isEnabled() and (
                self.reminder_dt_2.dateTime().toPyDateTime() - datetime.now()).total_seconds() < 15:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("Please set a time further than right now")
            msg.setWindowTitle("Wrong Input")
            ...
            msg.exec()
            retry = True
        elif self.reminder_chkbx_2.isChecked() and self.reminder_chkbx_2.isEnabled() :
